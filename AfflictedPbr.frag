 ...
 float noiseHash = getStaticNoiseVar0(wPosition, plaguedValue);
  ...
  //SUN EXPOSURE AND TIME OF DAY
   ...
   //     #endif
    ...
    #endif
   //    brightestPointLight = max(minVertLighting, brightestPointLight); //make sure lightprove always shows atleast a tiny bit, my setting a minimum value instead of 0.0 full dark 100 100 no see at all 
    
    factoredTimeOfDayScale = max(factoredTimeOfDayScale, minVertLighting); //essentially just the vertColors.r nultiplied by the time of day scale
        ...
        horiz *= horiz * factoredTimeOfDayScale ;

        vec3 indirectDiffuse = vec3(0.0);
        vec3 indirectSpecular = vec3(0.0);
        indirectDiffuse = sphericalHarmonics(normal.xyz, g_ShCoeffs) * diffuseColor.rgb * factoredTimeOfDayScale;
